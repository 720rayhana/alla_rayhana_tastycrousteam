...
# On supprime ici l'importation des Agents qu'on n'utilise pas
from agents.team6.agent6 import Agent6

from pystk2_gymnasium.envs import STKRaceMultiEnv, AgentSpec
from pystk2_gymnasium.definitions import CameraMode

MAX_TEAMS = 1 # Changement de la valeur pour observer un seul Agent
MAX_STEPS = 1000
NB_RACES = 1
...
def create_race():
    ...
    if NB_RACES==1:
        # Changement du track pour mieux observer le comportement de l'agent, sur abyss, on se cogne vite à un mur
        env = STKRaceMultiEnv(agents=agents_specs, track="scotland", render_mode="human", num_kart=MAX_TEAMS)
    else:
...
if __name__ == "__main__":
    ...
    # On empêche cette ligne de s'exécuter car provoque une erreur car le chemin n'existe pas (et on n'accorde pas d'importance au score de la course si on est seul)
